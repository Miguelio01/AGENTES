...
export class OrchestratorService {
  ...
  async handleIncomingMessage(
    ...
    if (this.isPureCatalogOrder(message)) {
      this.logger.log(`⚡ VÍA RÁPIDA: Procesando pedido de catálogo determinísticamente.`);
      const handled = await this.handleDirectCatalogOrder(message, client, session, replyCallback, presenceCallback);
      if (handled) return;
      this.logger.warn(`⚠️ VÍA RÁPIDA falló, reintentando por vía cognitiva (ADK).`);
    }
  ...
  private isPureCatalogOrder(message: Message): boolean {
    if (!message.metadata?.orderItems || message.metadata.orderItems.length === 0) return false;
    ...
    const isAutoGenerated = cleanContent.includes('nuevo pedido del catálogo') || cleanContent === '';
    const isSimpleGreeting = cleanContent === 'hola' || cleanContent === 'buenos días' || cleanContent === 'buenas tardes';
    
    return isAutoGenerated || isSimpleGreeting;
  }

  private async interceptWaitlistResponse(message: Message, client: Client, session: Session, replyCallback: any): Promise<boolean> {
    ...
    if ((isYes || isNo) && session.metadata?.missingItems) {
      if (isYes) {
        ...
        const reply = Message.create({ 
          content: 'Bueno señor, esperamos el comprobante de pago ya que no quiere el producto más adelante.', 
          role: 'assistant', channel: message.channel 
        });
        await replyCallback(reply);
        session.addMessage(message);
  ...
  private async handleDirectCatalogOrder(message: Message, client: Client, session: Session, replyCallback: any, presenceCallback: any): Promise<boolean> {
    const rawItems = message.metadata?.orderItems;
    if (!rawItems) return false;

    const orderId = session.metadata?.currentOrderId || `ORD-${Date.now().toString().slice(-6)}`;

    try {
      // 1. Verificar stock real (Fetch Once)
      const inventoryResponse = await this.inventoryAgent.handleRequest({
        from: 'orchestrator' as any,
        to: 'inventory-agent' as any,
        action: 'check_stock_batch' as any,
        data: { items: rawItems.map((i: any) => ({ productName: i.product || i.productName, quantity: i.quantity })) },
        context: { clientId: client.id }
      });

      if (inventoryResponse.status !== 'SUCCESS') throw new Error('Error al verificar inventario');
      ...
      // Guardar faltantes para la intercepción de lista de espera
      ...
      // 2. Obtener configuración (Domicilio)
      const config = await this.inventoryAgent.handleRequest({
        from: 'orchestrator' as any, to: 'inventory-agent' as any, action: 'get_config' as any, data: {}, context: { clientId: client.id }
      });
      const deliveryFee = parseInt(config.data['COSTO_DOMICILIO']?.replace(/[$. ]/g, '') || '0');

      // 3. Construir mensaje de respuesta (Formato WhatsApp)
      let subtotal = 0;
      let breakdown = availableItems.map((i: any) => {
        const itemTotal = i.pricePerUnit * i.availableQuantity;
        subtotal += itemTotal;
        return `•⁠  ⁠${i.availableQuantity}x ${i.productName} ($${itemTotal})`;
      }).join('\n');

      const total = subtotal + deliveryFee;

      // Persistencia crítica para la Saga de aprobación y notificaciones
      if (!session.metadata) session.metadata = {};
      session.metadata.total = total;
      session.metadata.deliveryFee = deliveryFee;
      session.metadata.currentOrderItems = availableItems.map((i: any) => ({
        productId: i.productId || i.productName,
        productName: i.productName,
      ...
      const reply = Message.create({ content: replyContent, role: 'assistant', channel: message.channel });
      
      // 5. Registrar en Prepago
      await this.registerCatalogueInPrepago(availableItems.map((i: any) => ({
        productId: i.productId || i.productName,
        productName: i.productName,
        quantity: i.availableQuantity,
      ...
      // Reconocer fase de pago para el estado de flujo
      ...
      this.logger.error(`❌ Error en vía rápida: ${e.message}`);
      return false;
    }
  }
  ...
  private isInitialContact(session: Session, message: Message): boolean {
    ...
    const cleanMsg = message.content.toLowerCase().trim();
    if (cleanMsg === 'si' || cleanMsg === 'no' || cleanMsg === 'sí') return false;
