...
export class TelegramOrdersService {
  ...
  async handleMessage(
    ...
    switch (session.flowState) {
      ...
      case 'TELEGRAM_AWAITING_CLIENT_DOC_TYPE':
        return this.handleAwaitingClientDocType(contentLow, session, replyCallback);

      case 'TELEGRAM_AWAITING_CLIENT_DOC_NUMBER':
        return this.handleAwaitingClientDocNumber(content, session, replyCallback);

      case 'TELEGRAM_AWAITING_CLIENT_ADDRESS':
        return this.handleAwaitingClientAddress(content, session, replyCallback);

      case 'TELEGRAM_AWAITING_CLIENT_EMAIL':
        return this.handleAwaitingClientEmail(contentLow, session, replyCallback);
      ...
      case 'TELEGRAM_AWAITING_DELIVERY_CHOICE':
        return this.handleAwaitingDeliveryChoice(contentLow, session, replyCallback);

      case 'AWAITING_PAYMENT_PROOF':
        const isMedia = !!message.metadata?.media;
        const isPaymentKeyword = contentLow.includes('pago') || contentLow.includes('soporte') || contentLow.includes('pagué') || contentLow.includes('listo');
        
        if (isMedia || isPaymentKeyword) {
           return this.handleAwaitingPaymentProof(message, session, replyCallback);
        }
        await replyCallback(Message.create({
  ...
  private async handleAwaitingClientName(name: string, session: Session, replyCallback: any) {
    if (!session.metadata) session.metadata = {};
    ...
    if (existingClient && existingClient.documentNumber && existingClient.address && existingClient.email) {
      return this.skipToProductsWithClient(existingClient, session, replyCallback);
    }
  ...
  private async handleAwaitingClientPhone(phone: string, session: Session, replyCallback: any) {
    const cleanPhone = phone.replace(/[^0-9]/g, '');
    if (cleanPhone.length < 10) {
      await replyCallback(Message.create({
        content: '⚠️ El teléfono parece inválido. Por favor envíe los 10 dígitos:',
        role: 'assistant',
        channel: 'telegram-orders'
      }));
      return true;
  ...
  private async skipToProductsWithClient(client: Client, session: Session, replyCallback: any) {
    this.logger.log(`👤 Cliente recurrente detectado: ${client.name}. Omitiendo recolección de datos.`);
    ...
    await replyCallback(Message.create({
      content: `✅ ¡Cliente encontrado! Se usarán los datos de *${client.fullName || client.name}*.\n` +
  ...
  private async handleAwaitingClientDocType(type: string, session: Session, replyCallback: any) {
  ...
  private async handleAwaitingClientDocNumber(num: string, session: Session, replyCallback: any) {
  ...
  private async handleAwaitingClientAddress(address: string, session: Session, replyCallback: any) {
  ...
  private async handleAwaitingClientEmail(email: string, session: Session, replyCallback: any) {
    session.metadata!.telegramOrder.email = email === 'n/a' ? `manual_${Date.now()}@frescoh.com` : email.trim();
    session.setFlowState('TELEGRAM_AWAITING_ORDER_ITEMS');
    await this.sessionsService.update(session);
  ...
  private async handleAwaitingOrderItems(itemsText: string, session: Session, replyCallback: any) {
    if (!session.metadata) session.metadata = {};
    if (!session.metadata.telegramOrder) session.metadata.telegramOrder = {};
  ...
  private async handleAwaitingDeliveryChoice(choice: string, session: Session, replyCallback: any) {
    ...
    if (!client) {
      client = Client.create(clientPhone, session.metadata.telegramOrder.clientName, clientPhone);
    ...
    const rawItems = session.metadata.telegramOrder.itemsRaw.split(/[\n,]+/).map((s: string) => s.trim()).filter(Boolean);
    ...
    const resolvedItems = rawItems.map((text: string) => {
        ...
        if (!found) {
            found = allProducts.find(p => p.name.toLowerCase().includes(searchLow) || searchLow.includes(p.name.toLowerCase()));
        }
        ...
        return { productId: 'MANUAL', productName: inputToSearch, quantity: qty, pricePerUnit: 0 }; 
    });
  ...
  private async handleAwaitingPaymentProof(message: Message, session: Session, replyCallback: any) {
    ...
    this.logger.log(`💰 Telegram Orders: Procesando comprobante para ${orderId}`);
  ...
  private async sendProductCatalogList(replyCallback: any) {
    ...
    try {
        ...
        availableProductsText = inventory
            .filter(p => p.stock > 0)
            .map(p => `• *${p.id}* | ${p.name} ($${p.price.toLocaleString('es-CO')})`)
            .join('\n');
    } catch (e) {}
